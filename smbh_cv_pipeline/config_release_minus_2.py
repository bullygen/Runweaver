...
class PipelineConfig:
    ...
    d1_edge_method: str = "sobel"            # log_mad | dog | sobel | laplace_sign
    d1_threshold_method: str = "sauvola"           # mad | percentile | sauvola
    d1_morphology_method: str = "open_close"          # none | open_close | remove_small
    d1_edge_sigmas: str = "1.0,1.5,2.0,3.0"
    d1_tau: float = 3.0
    d1_percentile: float = 98.0
    d1_min_area: int = 4
    d1_skeletonize: bool = True
    d1_jet_percentile: float = 99.7
    d1_rect_half_size: int = 6

    # D2
    d2_method: str = "sparse_rht"   # sparse_rht | sparse_rht_stratified
    d2_vote_weight: str = "none"               # none | mean | product
    d2_votes_fraction: float = 0.2             # fraction of N^3, capped by d2_max_votes
    d2_max_votes: int = 200000
    ...
    # D6
    d6_method: str = "merge_delta_bic"          # amplitude | delta_bic | merge_delta_bic | none
    d6_min_artifacts: int = 0
    d6_amp_min: float = 0.003
    d6_delta_bic_threshold: float = 0.0
    d6_merge_center_px: float = 3.0
    d6_merge_radius_px: float = 3.0
